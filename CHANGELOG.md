...
- Semilla controlada de reconciliación AEAT integrada en `aeat:gate`: después de una única remisión real `accepted`, puede crear una SQLite privada `0600` en `reconciliation_required` con la misma identidad/`RefExterna`/huella y un fichero operador privado; exige `AEAT_RECONCILIATION_SEED=YES`, destinos nuevos y commit SHA40, reserva los destinos antes de cargar PFX/red, añade cero llamadas `submit()` y no publica rutas privadas ni job ID.
